...
const BARE_RPC_SWIFT_VERSION = '1.0.0'
...
let package = Package(
  ...
  dependencies: [
    .package(url: "${BARE_RPC_SWIFT_URL}", from: "${BARE_RPC_SWIFT_VERSION}"),
  ...
  targets: [
    .target(
      ...
      dependencies: [
        ...
        .product(name: "${schemaPackageName.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}", package: "${schemaPackageId.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}")
      ],
      path: "Sources"
    )
  ]
